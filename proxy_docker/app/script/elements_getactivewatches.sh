...
elements_get_txns_by_watchlabel(){
  ...
  query=$(cat <<-HERE
	...
	INNER JOIN elements_watching AS w ON w32.id = w.elements_watching_by_pub32_id
	INNER JOIN elements_watching_tx AS wtxn ON w.id = wtxn.elements_watching_id
	INNER JOIN elements_tx AS tx ON wtxn.elements_tx_id = tx.id
	WHERE w32.label='${1}'
	LIMIT ${2-10} OFFSET 0
HERE
  )
...
elements_get_unused_addresses_by_watchlabel(){
  ...
  query=$(cat <<-HERE
        ...
        INNER JOIN elements_watching_by_pub32 AS w32 ON w.elements_watching_by_pub32_id = w32.id
        WHERE w32.label='${1}'
        AND NOT EXISTS (
...
elements_getactivewatches() {
  ...
  watches=$(sql "SELECT '{\"id\":' || id || ',\"address\":\"' || address || '\",\"unblindedAddress\":\"' || unblinded_address || '\",\"watchingAssetId\":\"' || watching_assetid || '\",\"imported\":' || imported || ',\"unconfirmedCallbackURL\":' || CASE WHEN callback0conf IS NULL THEN 'null' ELSE ('\"' || callback0conf || '\"') END || ',\"confirmedCallbackURL\":' || CASE WHEN callback1conf IS NULL THEN 'null' ELSE ('\"' || callback1conf || '\"') END || ',\"label\":\"' || COALESCE(label, '') || '\",\"watching_since\":\"' || inserted_ts || '\"}' FROM elements_watching WHERE watching AND NOT calledback1conf ORDER BY id")
  returncode=$?
  trace_rc ${returncode}
...
elements_getactivewatchesxpub() {
  ...
  watches=$(sql "SELECT '{\"id\":' || w.id || ',\"address\":\"' || address || '\",\"unblindedAddress\":\"' || unblinded_address || '\",\"watchingAssetId\":\"' || watching_assetid || '\",\"imported\":' || imported || ',\"unconfirmedCallbackURL\":' || CASE WHEN w.callback0conf IS NULL THEN 'null' ELSE ('\"' || w.callback0conf || '\"') END || ',\"confirmedCallbackURL\":' || CASE WHEN w.callback1conf IS NULL THEN 'null' ELSE ('\"' || w.callback1conf || '\"') END || ',\"watching_since\":\"' || w.inserted_ts || '\",\"derivation_path\":\"' || derivation_path || '\",\"pub32_index\":' || pub32_index || '}' FROM elements_watching w, elements_watching_by_pub32 w32 WHERE elements_watching_by_pub32_id = w32.id AND w32.${where} = '${value}' AND w.watching AND NOT calledback1conf ORDER BY w.id")
  returncode=$?
  trace_rc ${returncode}
