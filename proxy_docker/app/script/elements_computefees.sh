...
elements_compute_fees() {
	local pruned="false"
	if [ "${pruned}" = "true" ]; then
		trace "[elements_compute_fees]  pruned=${pruned}"
...
elements_compute_vin_total_amount() {
	...
	do
		...
		vin_raw_tx=$(sql "SELECT raw_tx FROM elements_tx WHERE txid=\"${vin_txid}\"")
		if [ -z "${vin_raw_tx}" ]; then
			txid_already_inserted=false
			vin_raw_tx=$(elements_get_rawtransaction "${vin_txid}")
			returncode=$?
		...
		if ! ${txid_already_inserted}; then
			...
			sql "INSERT OR IGNORE INTO elements_tx (txid, hash, confirmations, timereceived, size, vsize, blockhash, blockheight, blocktime, raw_tx) VALUES (\"${vin_txid}\", ${vin_hash}, ${vin_confirmations}, ${vin_timereceived}, ${vin_size}, ${vin_vsize}, ${vin_blockhash}, ${vin_blockheight}, ${vin_blocktime}, readfile('rawtx-${vin_txid}.blob'))"
			trace_rc $?
