...
if [ -n "$table_descr" ]; then
  ...
  SQL_ST="CREATE INDEX idx_elements_watching_by_txid_1x ON elements_watching_by_txid (txid, COALESCE(callback1conf, ''::character varying), COALESCE(callbackxconf, ''::character varying))"
  trace "[$SCRIPT_NAME] $SQL_ST"
  psql -qAtX -h postgres -U cyphernode -c "$SQL_ST"
  returncode=$?
  trace_rc ${returncode}
