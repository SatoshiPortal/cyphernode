...
start_test_container() {
  docker run -d --rm -t --name tests-watch-pub32 --network=cyphernodenet alpine:3.15.4
}
...
start_callback_server() {
  ...
  docker run --rm -t --name tests-watch-pub32-cb --network=cyphernodenet alpine:3.15.4 sh -c "nc -vlp${port} -e sh -c 'echo -en \"HTTP/1.1 200 OK\\\\r\\\\n\\\\r\\\\n\" ; echo -en \"\\033[40m\\033[0;37m\" >&2 ; date >&2 ; timeout 1 tee /dev/tty | cat ; echo -e \"\033[0m\" >&2'" &
