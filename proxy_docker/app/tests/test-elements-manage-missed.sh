...
start_test_container() {
  docker run -d --rm -t --name tests-elements-manage-missed --network=cyphernodenet alpine:3.15.4
}
...
exec_in_test_container() {
  docker exec tests-elements-manage-missed "$@"
}
...
wait_for_proxy() {
  ...
  docker exec tests-elements-manage-missed sh -c 'while true ; do ping -c 1 proxy ; [ "$?" -eq "0" ] && break ; sleep 5; done'

  # Now check if it's ready to accecpt requests
  docker exec tests-elements-manage-missed sh -c 'while true ; do curl proxy:8888/helloworld > /dev/null; [ "$?" -eq "0" ] && break ; sleep 5; done'
}
...
wait_for_broker() {
  ...
  # First ping the containers to make sure they're up...
  docker exec tests-elements-manage-missed sh -c 'while true ; do ping -c 1 broker ; [ "$?" -eq "0" ] && break ; sleep 5; done'
}
...
test_elements_manage_missed_0_conf() {
  ...
  docker exec "$(elements_container)" elements-cli -rpcwallet=spending01.dat sendtoaddress ${address} 0.00001
...
test_elements_manage_missed_1_conf_dead_broker() {
  ...
  trace 1 "\n[test_elements_manage_missed_1_conf_dead_broker] ${BCyan}Let's miss a 1-conf with a dead broker!...${Color_Off}"
  ...
  trace 2 "[test_elements_manage_missed_1_conf_dead_broker] getnewaddress..."
  local response=$(exec_in_test_container curl -d '{"label":"missed0conftest"}' proxy:8888/elements_getnewaddress)
  trace 3 "[test_elements_manage_missed_1_conf_dead_broker] response=${response}"
  local address=$(echo "${response}" | jq -r ".address")
  trace 3 "[test_elements_manage_missed_1_conf_dead_broker] address=${address}"

  start_callback_server $port0 ${address} &
  start_callback_server $port1 ${address} &

  trace 2 "[test_elements_manage_missed_1_conf_dead_broker] watch it..."
  local data='{"address":"'${address}'","unconfirmedCallbackURL":"'${callbackurl0conf}'","confirmedCallbackURL":"'${callbackurl1conf}'","label":"missed0conftest"}'
  trace 3 "[test_elements_manage_missed_1_conf_dead_broker] data=${data}"
  response=$(exec_in_test_container curl -d "${data}" proxy:8888/elements_watch)
  trace 3 "[test_elements_manage_missed_1_conf_dead_broker] response=${response}"
  ...
  trace 3 "[test_elements_manage_missed_1_conf_dead_broker] Shutting down the broker..."
  ...
  trace 3 "[test_elements_manage_missed_1_conf_dead_broker] Calling executecallbacks..."
  exec_in_test_container curl -s -H "Content-Type: application/json" proxy:8888/executecallbacks
...
test_elements_manage_missed_0_conf_multiple_txids() {
  ...
  docker exec "$(elements_container)" elements-cli -rpcwallet=spending01.dat sendtoaddress ${address} 0.0001

  trace 3 "[test_elements_manage_missed_0_conf_multiple_txids] Sending coins again to watched address while proxy is down..."
  docker exec "$(elements_container)" elements-cli -rpcwallet=spending01.dat sendtoaddress ${address} 0.0002
