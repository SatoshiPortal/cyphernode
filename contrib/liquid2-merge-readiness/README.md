...
- `staging-server-live-smoke.sh`
  Runs live smoke tests against a deployed Cyphernode stack. This is safe for
  the staging server: it creates wallet addresses and temporary watch rows,
...
```sh
cd "$STACK_DIR"
/path/to/contrib/liquid2-merge-readiness/staging-server-live-smoke.sh
